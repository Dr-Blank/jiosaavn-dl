...
class Jiosaavn:
    ...
    def processAlbum(self, album_id, output_path="Downloads"):
        # sanitization
        ...
        for song in album_json["songs"]:
            ...
            self.processTrack(
                song_id, album_artist, song_pos, total_tracks, output_path=output_path
            )
            song_pos += 1
    ...
    def processTrack(
        ...
        album_path = os.path.join(output_path, folder_name)
        song_path = os.path.join(output_path, folder_name, song_name)
...
if __name__ == "__main__":
    ...
    url = args.url
    out_dir = args.output if args.output else r"D:\Users\ASUS\Music\~temp"
    ...
    if "/album/" in url or "/song/" in url:
        ...
        if kind == "song":
            jiosaavn.processTrack(id_, None, 1, 1, output_path=out_dir)
        elif kind == "album":
            jiosaavn.processAlbum(id_, output_path=out_dir)
