...
class Jiosaavn:
    ...
    def tagger(self, json, song_path, album_artist, album_path, pos=1, total=1):
        ...
        audio["aART"] = (
            album_artist
            if album_artist
            else sanitize(unescape(json["primary_artists"]))
        )
    ...
    def processUrl(self, url, out_dir="Downloads", archive_files:dict=None):
        """archive_files is a dictionary of archive files for faster processing"""
        ...
        if "/album/" in url or "/song/" in url:
            ...
            if kind == "song":
                self.processTrack(id_, None, 1, 1, out_dir=out_dir)

            elif kind == "album":
                self.processAlbum(
                    id_, out_dir=out_dir, archive_file=archive_files.get("album")
                )

        elif "/playlist/" in url:
            playlist_id = playlist_rx.search(url).group(1)
            self.processPlaylist(playlist_id, out_dir=out_dir)

        elif "/artist/" in url:
            artist_id = artist_rx.search(url).group(1)
            self.processArtist(artist_id, out_dir=out_dir, archive_files=archive_files)
        else:
    ...
    def processAlbum(
        ...
        out_dir="Downloads",
        archive_file=None,
        archive_list: set = None,
    ):
        """archive_file is the file where the album ids are stored and archive_list is the list of album ids of faster processing"""
        if archive_list is None:
            archive_list = set()
            if archive_file and os.path.exists(archive_file):
                with open(archive_file, "r", encoding="utf-8") as f:
                    archive_list = set(f.read().splitlines())

        if archive_list and album_id in archive_list:
            print(f"Album {album_id} already downloaded.")
            return
        ...
        for song in album_json["songs"]:
            ...
            self.processTrack(
                song_id, album_artist, song_pos, total_tracks, out_dir=out_dir
            )
            song_pos += 1

        # write the album id to the archive file for future reference
        archive_list.add(album_id)
        if archive_file:
    ...
    def processTrack(
        ...
        primary_artists = (
            album_artist
            if album_artist
            else sanitize(unescape(song_json["primary_artists"]))
        )
        ...
        album_path = os.path.join(out_dir, folder_name)
        song_path = os.path.join(out_dir, folder_name, song_name)
        ...
        if not os.path.exists(os.path.join(album_path, "cover.jpg")) or isPlaylist:
            ...
            with open(os.path.join(album_path, "cover.jpg"), "wb") as f:
                f.write(
                    self.session.get(song_json["image"].replace("150", "500")).content
                )
    ...
    def processPlaylist(self, playlist_id, out_dir="Downloads"):
        # json
    ...
    def processArtist(self, artist_id, out_dir="Downloads", archive_files:dict=None):
        """archive_files is a dictionary of archive files for faster processing"""
        if archive_files is None:
            archive_files = {}
        ...
        for album in artist_json["topAlbums"]["albums"]:
            ...
            self.processAlbum(id_, out_dir=out_dir, archive_file=archive_files.get("album"))
